...
app.post('/uploadfile', uploadMultiple, function (req, res, next) {
  var sommaire=req.body.sommaire
  ...
  if(req.files){
      ...
      PythonShell.run("scriptpython.py",options,function(err,resultat){
        if (err){
          ...
          let filePath1 = `http://localhost:5000/output/highlited-${name1}`;
          let filePath2 = `http://localhost:5000/output/highlited-${name2}`;
...
app.post('/upload', (req, res) => {
  ...
  file.mv(`${__dirname}/uploads/${file.name}`, err => {
    ...
    let options ={
      args :[place],
      scriptPath:"C:/Users/Hiba Zouita/scriptPython"
